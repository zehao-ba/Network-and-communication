...
#		address2_bin = address2_bin.zfill(7)
	...
	c.write( "destination is"+ str(destination) + '\n')
	...
	for i in range (0,4):
		...
		result.append(res)
	c.write( "And computation is" +str(result) +'\n')
	return result
	...
	for packet in packets:
		...
		#	print "this is the PACKET", packet
			if(compare(dest, result)):
				dest_match = True
				c.write( "Packet with destination address"+ dest + "will be forwarded to"+hop+"out on interface"+interface +'\n')
			else:
				continue

	if(not dest_match):
		c.write( "No destination matched. Please try other packet."+'\n')
